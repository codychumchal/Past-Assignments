// FILE: sequence.h
// TEMPLATE CLASS PROVIDED: seqItem<T> (a container class for a list of items,
//                 where each list may have a designated item called
//                 the current item)
//
// TYPEDEFS and MEMBER CONSTANTS for the seqItem<T> template class:
//   typedef ____ T
...
//     sequence can hold.
//
// CONSTRUCTOR for the seqItem<T> template class:
//   seqItem()
//     Pre:  (none)
//     Post: The sequence has been initialized as an empty sequence.
//
// MODIFICATION MEMBER FUNCTIONS for the seqItem<T> template class:
//   void start()
//     Pre:  (none)
...
//
// CONSTANT MEMBER FUNCTIONS for the seqItem<T> template class:
...
// VALUE SEMANTICS for the seqItem<T> template class:
//    Assignments and the copy constructor may be used with seqItem
//    objects.
